...
export async function fetchAuditLogs(limit = 100): Promise<AuditLog[]> {
  const { data } = await supabase
    .from("audit_logs")
    .select(
      "id,actor_id,action,target_type,target_id,metadata,created_at,actor:profiles!audit_logs_actor_id_fkey(username)",
    )
...
export async function submitCoinPurchase(input: {
  method_code: string;
  coins: number;
  amount_cents: number;
  currency: string;
...
export async function adminApproveCoinPurchase(id: string, note?: string) {
  const { error } = await supabase.rpc("admin_approve_coin_purchase", {
    _req_id: id,
    _note: note ?? undefined,
  });
  if (error) throw error;
...
export async function adminRejectCoinPurchase(id: string, note?: string) {
  const { error } = await supabase.rpc("admin_reject_coin_purchase", {
    _req_id: id,
    _note: note ?? undefined,
  });
  if (error) throw error;
...
export async function followUser(userId: string) {
  ...
  const { error } = await supabase
    .from("user_follows")
    .insert({ follower_id: u.user.id, followed_id: userId });
  if (error) throw error;
}
...
export async function fetchRecentlyViewed(limit = 12) {
  ...
  if (!u.user) {
    try {
      ...
      const { data } = await supabase
        .from("novels")
        .select("id,slug,title,cover_url,author")
