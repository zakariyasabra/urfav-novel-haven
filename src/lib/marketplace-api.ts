...
export async function mkCoinHistory(limit = 50, before?: string): Promise<CoinHistoryRow[]> {
  const { data } = await supabase.rpc("mk_coin_history", {
    _limit: limit,
...
export async function mkPurchaseHistory(limit = 30, before?: string): Promise<PurchaseRow[]> {
  const { data } = await supabase.rpc("mk_purchase_history", {
    _limit: limit,
    _before: before ?? undefined,
  });
...
export async function mkAdminGrantItem(userId: string, itemId: string) {
  const { data, error } = await supabase.rpc("mk_admin_grant_item", {
    _user: userId,
    _item_id: itemId,
  });
  if (error) throw error;
...
export const RARITY_STYLES: Record<
  Rarity,
  { ring: string; text: string; label_ar: string; glow: string }
> = {
