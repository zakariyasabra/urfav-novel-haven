...
export async function subscribePush(): Promise<{ ok: boolean; error?: string }> {
  ...
  try {
    ...
    const sub =
      ...
      (await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: undefined }));
...
export async function upsertEmailTemplate(
  t: Partial<EmailTemplate> & { code: string; name: string; subject_ar: string; body_ar: string },
) {
...
export async function toggleChapterReaction(chapterId: string, emoji: string, on: boolean) {
  ...
  if (on) {
    const { error } = await supabase
      .from("chapter_reactions")
      .insert({ user_id: u.user.id, chapter_id: chapterId, emoji });
...
export async function exportTable(
  entity: "novels" | "chapters" | "profiles" | "genres" | "comments",
): Promise<string> {
