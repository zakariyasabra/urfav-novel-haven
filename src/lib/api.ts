...
function resolveNovel(row: NovelRow, lang: Lang): Novel {
  return {
    ...
    original_title:
      pickText(
        row.original_title_ar,
        row.original_title_en,
        lang,
      ) || (row.original_title ?? null),
    author:
      pickText(
        row.author_display_ar,
        row.author_display_en,
        lang,
      ) || row.author,
    translator:
      pickText(
 ...
 */
async function applyChapterViewTotals(
  novels: Novel[],
): Promise<Novel[]> {
  ...
  /*
   * Preferred path:
   * PostgreSQL calculates SUM(views_count) internally and sends back
   * one row per novel instead of sending every chapter row to the app.
   */
  const { data, error } = await (supabase as any).rpc(
    "novel_chapter_view_totals",
    {
      _novel_ids: ids,
    },
  );
  ...
  if (!error) {
    ...
    for (const row of data ?? []) {
      const novelId = String(
        (row as { novel_id: string }).novel_id,
      );
  ...
  /*
   * Safety fallback:
   * If the RPC is temporarily unavailable, use the old calculation
   * so novel cards/details do not stop working.
   */
  console.warn(
    "[applyChapterViewTotals] RPC failed; falling back to chapter rows:",
    error,
  );
  ...
  let fallback = await base().is("deleted_at", null);

  if (
    fallback.error &&
    (
      fallback.error.code === "42703" ||
...
export async function fetchNovels(
  ...
  let q = supabase
    .from("novels")
    .select(NOVEL_CARD_COLS);

  if (opts.status) {
    q = q.eq(
      "status",
      opts.status as "ongoing" | "completed" | "hiatus",
    );
  }
  ...
  let novels = (
    (data ?? []) as unknown as NovelRow[]
  ).map(
    (r) => resolveNovel(r, lang),
  );
...
/**
 * Fetch novels for a set of IDs
 * (used by recommendation hydration).
...
export async function fetchNovelBySlug(
  ...
  const { data, error } = await supabase
    .from("novels")
    .select(
      `${NOVEL_FULL_COLS}, novel_genres(genre:genres(id,slug,name_ar,name_en))`,
    )
    .eq("slug", slug)
    .maybeSingle();
  ...
  const base = resolveNovel(
    row,
    lang,
  );

  const [withChapterViews] =
    await applyChapterViewTotals(
      [base],
    );
...
export async function fetchChapters(
  ...
  // القائمة العامة: الفصول المنشورة فقط
  // (بدون المسودات/المجدولة/المحذوفة)
  const base = () =>
    supabase
      .from("chapters")
      .select(
        "id,chapter_number,title,title_ar,title_en,is_vip,views_count,created_at,status",
      )
      .eq("novel_id", novelId)
      .eq("status", "published")
      .order(
  ...
  return (
    ...
    (
      ...
      title:
        pickText(
          c.title_ar as string | null,
          c.title_en as string | null,
          lang,
        ) ||
        (c.title as string),
...
export async function fetchChapter(
  ...
  const chapter = {
    ...
    content:
      pickText(
        row.content_ar as string | null,
        row.content_en as string | null,
        lang,
      ) ||
  ...
  return (
    ...
    (
      ...
      return {
        ...
        title:
          pickText(
            r.title_ar as string | null,
            r.title_en as string | null,
            lang,
          ) ||
          (r.title as string),
        ...
        novel: {
          ...
          title:
            pickText(
              n.title_ar as string | null,
              n.title_en as string | null,
              lang,
            ) ||
            (n.title as string),
...
export async function searchNovels(
  ...
  if (filters.genre) {
    const {
      ...
      .from("novel_genres")
      .select(
        "novel_id, genre:genres!inner(slug)",
      )
      .eq(
        "genre.slug",
        filters.genre,
...
export async function fetchNovelsByGenre(
  genreSlug: string,
  lang: Lang = currentLang(),
) {
  const {
  ...
  return applyChapterViewTotals(
    novels,
  );
}

// View recording now goes through record_* RPCs
// (real event rows + dedup).
export async function incrementNovelView(
  id: string,
) {
  const {
    recordNovelView,
