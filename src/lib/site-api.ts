...
export async function fetchSiteSettings(): Promise<Record<string, Record<string, unknown>>> {
  ...
  for (const row of (data ?? []) as { key: string; value: Record<string, unknown> }[]) out[row.key] = row.value;
  return out;
}

export async function submitReport(payload: {
  type: "contact" | "dmca" | "abuse" | "bug" | "comment";
  reporter_email?: string; reporter_name?: string; subject?: string; content: string;
  target_url?: string; target_id?: string;
}) {
