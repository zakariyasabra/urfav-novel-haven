...
export function AnnouncementBanner() {
  ...
  return (
    <div className="relative z-40 bg-gradient-to-r from-primary via-primary-glow to-primary py-2 text-center text-sm text-primary-foreground">
      {first.link_url ? (
        <a href={first.link_url} target="_blank" rel="noreferrer" className="hover:underline">
          {body}
        </a>
      ) : (
        body
...
export function AnnouncementPopup() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem(DISMISS_KEY + "_popup");
      if (raw) setDismissed(new Set(JSON.parse(raw)));
    } catch {
      /* ignore */
    }
  ...
  const q = useQuery({
    ...
    queryFn: () => fetchAnnouncements("popup"),
    staleTime: 60_000,
  });
  useEffect(() => {
    const first = (q.data ?? []).find((a) => !dismissed.has(a.id));
    if (first) {
      const t = setTimeout(() => setOpenId(first.id), 1500);
  ...
  function close() {
    ...
    setDismissed(next);
    try {
      localStorage.setItem(DISMISS_KEY + "_popup", JSON.stringify([...next]));
    } catch {
      /* ignore */
    }
    setOpenId(null);
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-md rounded-2xl border border-primary/40 bg-surface p-6 shadow-elevated"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={close}
          className="absolute end-3 top-3 grid h-8 w-8 place-items-center rounded-full hover:bg-secondary"
        >
          <X className="h-4 w-4" />
        </button>
