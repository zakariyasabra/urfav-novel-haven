    ...
    <div className="space-y-4">
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-full border px-4 py-1.5 text-xs font-semibold ${tab === "items" ? "border-primary bg-primary text-primary-foreground" : "border-border/50"}`}
        >
          العناصر
        </button>
        <button
          onClick={() => setTab("economy")}
          className={`rounded-full border px-4 py-1.5 text-xs font-semibold ${tab === "economy" ? "border-primary bg-primary text-primary-foreground" : "border-border/50"}`}
        >
          لوحة الاقتصاد
        </button>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button
          onClick={rotate}
          className="inline-flex items-center gap-1 rounded-lg border border-primary/40 px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary/10"
        >
              ...
              items.map((it) => (
                <tr key={it.id} className="border-t border-border/30">
                  ...
                  <td className="p-2 text-xs text-muted-foreground">
                    {CATEGORY_LABELS_AR[it.category]}
                  </td>
                  <td className={`p-2 text-xs ${RARITY_STYLES[it.rarity].text}`}>
                    {RARITY_STYLES[it.rarity].label_ar}
                  </td>
...
function ItemEditor({
  ...
  cats: MarketCategoryRow[];
  onClose: () => void;
  onSaved: () => void;
}) {
  const [f, setF] = useState<Partial<MarketItem>>({ ...item });
  const [saving, setSaving] = useState(false);
  const set = <K extends keyof MarketItem>(k: K, v: MarketItem[K] | null) =>
    setF((p) => ({ ...p, [k]: v as never }));

  async function save() {
    setSaving(true);
    try {
      if (!f.title_ar || !f.category || !f.code) {
        toast.error("العنوان والكود والفئة مطلوبة");
        return;
      }
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-border/40 bg-card p-5"
      >
        <h3 className="mb-4 flex items-center gap-2 text-lg font-black">
          <Sparkles className="h-5 w-5 text-primary" /> {item.id ? "تعديل عنصر" : "عنصر جديد"}
        </h3>
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <Field label="كود فريد">
            <input
              value={f.code ?? ""}
              onChange={(e) => set("code", e.target.value)}
              className={inputCls}
              placeholder="frame_gold"
            />
          </Field>
          <Field label="الفئة">
            <select
          ...
          <Field label="السعر (عملات)">
            <input
              type="number"
              value={f.price_coins ?? 0}
              onChange={(e) => set("price_coins", +e.target.value)}
              className={inputCls}
            />
          </Field>
          <Field label="المدة (أيام، فارغ = دائم)">
            <input
              type="number"
              value={f.duration_days ?? ""}
              onChange={(e) => set("duration_days", e.target.value ? +e.target.value : null)}
              className={inputCls}
            />
          </Field>
          <Field label="المخزون (فارغ = غير محدود)">
            <input
              type="number"
              value={f.stock ?? ""}
              onChange={(e) => set("stock", e.target.value ? +e.target.value : null)}
              className={inputCls}
            />
          </Field>
          <Field label="حد لكل مستخدم">
            <input
              type="number"
              value={f.max_per_user ?? ""}
              onChange={(e) => set("max_per_user", e.target.value ? +e.target.value : null)}
              className={inputCls}
            />
          </Field>
          ...
          <div className="col-span-2">
            <Field label="Payload JSON (frame_code / title_code / vip_days / lifetime …)">
              <textarea
                ...
                onChange={(e) => {
                  try {
                    set("payload", JSON.parse(e.target.value));
                  } catch {
                    /* ignore */
                  }
                }}
          ...
          <label className="flex items-center gap-2">
            <input
              type="checkbox"
              checked={!!f.vip_only}
              onChange={(e) => set("vip_only", e.target.checked)}
            />{" "}
            VIP فقط
          </label>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <Section title="أعلى المشترين" icon={<BarChart3 className="h-4 w-4" />}>
        <List
          rows={data.top_buyers.map((b) => ({
            a: b.user_id.slice(0, 8),
            b: `${b.purchases} عملية`,
            c: b.spent.toLocaleString() + " عملة",
          }))}
        />
      </Section>
      <Section title="أعلى الكاسبين" icon={<BarChart3 className="h-4 w-4" />}>
        <List
          rows={data.top_earners.map((b) => ({
            a: b.user_id.slice(0, 8),
            b: "",
            c: b.earned.toLocaleString() + " عملة",
          }))}
        />
      </Section>
      <Section title="الأكثر شراءً" icon={<BarChart3 className="h-4 w-4" />}>
        <List
          rows={data.most_purchased.map((m) => ({
            a: m.title_ar || "—",
            b: `${m.purchases} عملية`,
            c: m.revenue.toLocaleString() + " عملة",
          }))}
        />
      </Section>
