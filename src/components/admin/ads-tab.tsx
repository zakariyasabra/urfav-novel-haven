...
const SLOT_KEYS = ["home_top","home_middle","home_bottom","sidebar","reader_top","reader_middle","reader_bottom","footer","banner"] as const;
...
export function AdsTab() {
  ...
  async function del(id: string) {
    if (!(await confirmDialog({ title: t("admin.confirm"), body: t("adsT.deleteBody"), confirmLabel: t("admin.confirm"), danger: true }))) return;
    await deleteAd(id); toast.success(t("adsT.deleted"));
    qc.invalidateQueries({ queryKey: ["ads-admin"] });
  }
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between">
        ...
        <Button size="sm" onClick={() => setEditing("new")}><Plus className="me-1 h-4 w-4" />{t("adsT.new")}</Button>
      </div>
      <div className="space-y-2">
          ...
          <div key={a.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-border/40 bg-surface/40 p-3">
            <div className="min-w-0">
              <div className="truncate font-bold">
                {SLOT_KEYS.includes(a.slot as typeof SLOT_KEYS[number]) ? t(`adsT.slot.${a.slot}`) : a.slot}
                <span className="ms-2 rounded bg-secondary px-1.5 py-0.5 text-[10px]">{KIND_KEYS.includes(a.kind as typeof KIND_KEYS[number]) ? t(`adsT.kind.${a.kind}`) : a.kind}</span>
              </div>
        ...
        {(q.data?.length ?? 0) === 0 && <div className="rounded-lg border border-dashed p-8 text-center text-sm text-muted-foreground">{t("adsT.empty")}</div>}
      </div>
...
function AdForm({ initial, onClose }: { initial: AdRow | null; onClose: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      toast.success(t("adsT.saved")); onClose();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl border border-border/60 bg-surface p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="mb-4 text-xl font-black">{initial ? t("adsT.editTitle") : t("adsT.newTitle")}</h3>
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            ...
            <select value={f.slot} onChange={(e) => setF({ ...f, slot: e.target.value })}
              ...
              {SLOT_KEYS.map((k) => <option key={k} value={k}>{t(`adsT.slot.${k}`)}</option>)}
            </select>
          </div>
          <div>
            ...
            <select value={f.kind} onChange={(e) => setF({ ...f, kind: e.target.value })}
              className="h-10 w-full rounded-md border border-input bg-background/60 px-3 text-sm">
              {KIND_KEYS.map((k) => <option key={k} value={k}>{t(`adsT.kind.${k}`)}</option>)}
            </select>
          </div>
          ...
          {(f.kind === "image" || f.kind === "native") && (
            <>
              <div className="md:col-span-2">
                <label className="mb-1 block text-xs font-semibold">{t("adsT.field.imageUrl")}</label>
                <input value={f.image_url} onChange={(e) => setF({ ...f, image_url: e.target.value })}
                  className="h-10 w-full rounded-md border border-input bg-background/60 px-3 text-sm" />
              </div>
              <div className="md:col-span-2">
                <label className="mb-1 block text-xs font-semibold">{t("adsT.field.linkUrl")}</label>
