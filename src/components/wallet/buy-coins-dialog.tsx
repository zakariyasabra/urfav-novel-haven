    ...
    <button
      ...
      onClick={async () => {
        try {
          ...
          setOk(true);
          toast.success(t("bc.copied"));
          setTimeout(() => setOk(false), 1500);
        } catch {
          toast.error(t("bc.copyErr"));
        }
      }}
...
export function MethodDetails({ m }: { m: PaymentMethod }) {
  ...
  useEffect(() => {
    let live = true;
    if (m.qr_image_url)
      signedQrUrl(m.qr_image_url).then((u) => {
        if (live) setQr(u);
      });
    return () => {
      live = false;
  ...
  if (m.code === "usdt") {
    if (c.address)
      rows.push({ label: t("bc.d.usdt", { n: c.network || "TRC20" }), value: c.address });
  }
  if (m.code === "vodafone_cash" && c.number)
    rows.push({ label: t("bc.d.vodafone"), value: c.number });
  if (m.code === "instapay" && c.handle) rows.push({ label: t("bc.d.instapay"), value: c.handle });
  if (rows.length === 0 && m.account_details)
    rows.push({ label: t("bc.d.generic"), value: m.account_details });

  return (
    <div className="space-y-2 rounded-lg border border-border/40 bg-background/40 p-3 text-xs">
      {m.instructions && (
        <div className="whitespace-pre-wrap text-foreground/85">{m.instructions}</div>
      )}
      ...
      {qr && (
        <div className="flex flex-col items-center gap-1 pt-1">
          <img
            src={qr}
            alt="QR"
            className="h-40 w-40 rounded-md border border-border/40 bg-white p-1"
          />
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-black sm:text-xl">
            {t("bc.title", { n: coins.toLocaleString(locale) })}
          </h3>
          <button onClick={onClose} aria-label={t("bc.close")}>
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="mb-4 rounded-lg border border-primary/40 bg-primary/10 p-3 text-sm">
          <div>
            {t("bc.suggested")}: <b>{formatMoney(suggestedCents, displayCurrency)}</b>
          </div>
          <div className="flex items-center gap-1 text-primary">
            <Coins className="h-4 w-4" /> {coins.toLocaleString(locale)}
          </div>
        </div>
            ...
            >
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  {m.code === "usdt" && m.config?.network
                    ? `USDT · ${m.config.network}`
                    : t(`pm.kind.${m.kind}` as never)}
                </div>
              </div>
              <span className="rounded-md bg-background/70 px-2 py-0.5 text-[10px] font-bold text-muted-foreground">
                {m.currency}
              </span>
            </button>
          ))}
        ...
        <label className="mb-1 block text-xs font-bold">
          {t("bc.amount", { c: displayCurrency === "EGP" ? t("bc.egp") : t("bc.usd") })}
        </label>
        <input
          value={amount}
          ...
          >
            <div className="min-w-0">
              <div className="truncate font-bold">
                {t("purchases.viaCoins", { n: r.coins.toLocaleString(locale), m: r.method_code })}
              </div>
              <div className="text-xs text-muted-foreground">
            ...
            >
              {r.status === "approved"
                ? t("purchases.st.approved")
                : r.status === "rejected"
                  ? t("purchases.st.rejected")
                  : t("purchases.st.pending")}
            </span>
          </div>
        ))}
      </div>
