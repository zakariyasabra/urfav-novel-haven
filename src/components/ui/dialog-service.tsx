...
type PromptOpts = {
  title: string; body?: string; label?: string; placeholder?: string; defaultValue?: string;
  multiline?: boolean; required?: boolean; maxLength?: number; confirmLabel?: string; cancelLabel?: string;
};
...
function push(r: Req) { for (const l of listeners) l(r); }
...
export function DialogHost() {
  ...
  function done() { setQueue(q => q.slice(1)); }

  if (!current) return null;

  if (current.kind === "confirm") {
    return <ConfirmUI opts={current.opts} onDone={(v) => { current.resolve(v); done(); }} />;
  }
  return <PromptUI opts={current.opts} onDone={(v) => { current.resolve(v); done(); }} />;
}
...
function ConfirmUI({ opts, onDone }: { opts: ConfirmOpts; onDone: (v: boolean) => void }) {
  return (
    <Shell title={opts.title} onClose={() => onDone(false)}>
      {opts.body && <p className="mb-5 text-sm text-muted-foreground whitespace-pre-line">{opts.body}</p>}
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => onDone(false)}>{opts.cancelLabel ?? "إلغاء"}</Button>
...
function PromptUI({ opts, onDone }: { opts: PromptOpts; onDone: (v: string | null) => void }) {
  ...
  function submit() { if (!valid) return; onDone(val); }
  return (
    <Shell title={opts.title} onClose={() => onDone(null)}>
      {opts.body && <p className="mb-3 text-sm text-muted-foreground whitespace-pre-line">{opts.body}</p>}
      {opts.label && <label className="mb-1 block text-xs font-bold">{opts.label}</label>}
      {opts.multiline ? (
        <textarea value={val} onChange={(e) => setVal(e.target.value)}
          maxLength={opts.maxLength ?? 500} placeholder={opts.placeholder} autoFocus
        ...
        <input value={val} onChange={(e) => setVal(e.target.value)}
          maxLength={opts.maxLength ?? 200} placeholder={opts.placeholder} autoFocus
      ...
      <div className="flex justify-end gap-2">
        <Button variant="outline" onClick={() => onDone(null)}>{opts.cancelLabel ?? "إلغاء"}</Button>
        <Button onClick={submit} disabled={!valid}>{opts.confirmLabel ?? "تأكيد"}</Button>
      </div>
    </Shell>
  );
}
