...
async function fetchForSection(s: HomepageSection): Promise<Novel[]> {
  ...
  switch (s.algorithm) {
    case "latest":
      return fetchNovels({ sort: "latest", limit: lim });
    case "popular":
      return fetchNovels({ sort: "popular", limit: lim });
    case "top_rated":
      return fetchNovels({ sort: "rating", limit: lim });
    case "completed":
      return fetchNovels({ status: "completed", sort: "popular", limit: lim });
    case "ongoing":
      return fetchNovels({ status: "ongoing", sort: "latest", limit: lim });
    case "trending":
      return fetchNovels({ sort: "popular", limit: lim });
    case "upcoming":
      return fetchNovels({ sort: "newest", limit: lim });
    case "random": {
      const all = await fetchNovels({ sort: "latest", limit: 60 });
      return [...all].sort(() => Math.random() - 0.5).slice(0, lim);
    }
    case "genre":
      return s.genre_slug ? (await fetchNovelsByGenre(s.genre_slug)).slice(0, lim) : [];
    default:
      return fetchNovels({ sort: "latest", limit: lim });
  }
}
...
function DynamicSection({ section }: { section: HomepageSection }) {
  ...
  const viewAll =
    section.algorithm === "genre" && section.genre_slug
      ? `/categories/${section.genre_slug}`
      : section.algorithm === "popular" || section.algorithm === "trending"
        ? "/popular"
        : section.algorithm === "completed"
          ? "/completed"
          : section.algorithm === "ongoing"
            ? "/ongoing"
            : "/latest";
  return (
    <section>
      <div className="mb-5 flex items-center justify-between gap-4">
        ...
        <Link
          to={viewAll}
          className="inline-flex shrink-0 items-center gap-1 text-sm font-semibold text-primary hover:text-primary-glow"
        >
