...
import { CHAPTER_EMOJIS, fetchChapterReactionCounts, fetchMyChapterReactions, toggleChapterReaction } from "@/lib/enterprise-api";
...
export function ChapterReactionsBar({ chapterId }: { chapterId: string }) {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      const [c, m] = await Promise.all([fetchChapterReactionCounts(chapterId), fetchMyChapterReactions(chapterId)]);
      if (!alive) return;
  ...
  async function react(emoji: string) {
    ...
    try {
      ...
      setMine(prev => { const s = new Set(prev); if (on) s.add(emoji); else s.delete(emoji); return s; });
  ...
  return (
    <div className="my-6 flex flex-wrap items-center justify-center gap-2 rounded-xl border border-border/40 bg-surface/40 p-3">
      <span className="me-2 text-xs font-semibold text-muted-foreground">{t("reactions.title")}</span>
      {CHAPTER_EMOJIS.map(e => (
        <button key={e} onClick={() => react(e)} disabled={busy === e}
          className={`inline-flex items-center gap-1 rounded-full border px-3 py-1.5 text-sm transition-colors ${
            mine.has(e) ? "border-primary bg-primary/10" : "border-border/60 bg-background/60 hover:bg-secondary/60"
          }`}>
          <span>{e}</span>
