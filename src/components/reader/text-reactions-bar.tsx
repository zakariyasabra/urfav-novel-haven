...
export function TextReactionsBar({ chapterId }: { chapterId: string }) {
  const q = useQuery({ queryKey: ["text-reactions", chapterId], queryFn: () => fetchChapterReactions(chapterId) });
  ...
  return (
    <aside className="mt-10 rounded-xl border border-border/40 bg-surface/40 p-4">
      <div className="mb-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">أكثر المقاطع تفاعلاً</div>
          ...
          <div key={i} className="rounded-lg border border-border/40 bg-background/40 p-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {Object.entries(g.counts).map(([emoji, n]) => (
                <span key={emoji} className="inline-flex items-center gap-1 rounded-full bg-secondary px-2 py-0.5 text-xs">
