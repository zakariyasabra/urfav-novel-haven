...
export function GiftCoinsButton({ authorId, novelId, authorName }: { authorId: string; novelId?: string; authorName?: string }) {
  const [open, setOpen] = useState(false);
  return (
    <>
      <Button size="sm" variant="outline" onClick={() => setOpen(true)}>
        <Gift className="me-1 h-4 w-4" />إهداء عملات
      </Button>
      {open && <GiftDialog authorId={authorId} novelId={novelId} authorName={authorName} onClose={() => setOpen(false)} />}
    </>
  );
}

function GiftDialog({ authorId, novelId, authorName, onClose }: { authorId: string; novelId?: string; authorName?: string; onClose: () => void }) {
  const { user } = useAuth();
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      await giftCoinsToAuthor({ author_id: authorId, amount, novel_id: novelId, message: message.trim() || null });
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl border border-border/60 bg-surface p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="mb-3 block">
          ...
          <input type="number" min={1} value={amount} onChange={(e) => setAmount(Math.max(1, parseInt(e.target.value) || 0))}
        ...
        {!user ? (
          <Button asChild className="w-full"><Link to="/auth">سجل الدخول لإرسال هدية</Link></Button>
        ) : canAfford ? (
          <Button onClick={send} disabled={busy || amount < 1} className="w-full bg-gradient-to-r from-primary to-primary-glow text-primary-foreground">
            {busy ? "جارٍ الإرسال..." : `إرسال ${amount.toLocaleString("ar")} عملة`}
          </Button>
        ) : (
          <Button asChild variant="outline" className="w-full"><Link to="/wallet">شحن العملات</Link></Button>
        )}
      </div>
    </div>
  );
