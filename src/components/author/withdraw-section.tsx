  ...
  return (
    <section className="mt-6 rounded-2xl border border-border/40 bg-surface/40 p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <Button size="sm" onClick={() => setOpen(true)} disabled={pending < 100}>{t("wd.new")}</Button>
      </div>
      ...
      <div className="space-y-2">
        {(listQ.data ?? []).map(w => (
          <div key={w.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-border/40 bg-background/40 p-3 text-sm">
            <div>
              <div><b>{t("wd.viaCoins", { n: w.coins.toLocaleString(locale), m: methodName(w.method_code) })}</b></div>
              <div className="text-xs text-muted-foreground">{new Date(w.created_at).toLocaleString(locale)}</div>
              {w.admin_note && <div className="text-xs">{t("wd.note")}: {w.admin_note}</div>}
            </div>
            ...
            }`}>{w.status === "approved" ? t("wd.st.approved") : w.status === "rejected" ? t("wd.st.rejected") : t("wd.st.pending")}</span>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-2xl border border-border/60 bg-surface p-6" onClick={e => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setOpen(false)} aria-label={t("bc.close")}><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="mb-3 flex items-center gap-1 text-xs text-muted-foreground"><Coins className="h-3 w-3" /> {t("wd.dlg.available", { n: pending.toLocaleString(locale) })}</div>
            ...
            <Button disabled={busy} onClick={submit} className="w-full">{t("wd.dlg.submit")}</Button>
          </div>
        </div>
      )}
    </section>
