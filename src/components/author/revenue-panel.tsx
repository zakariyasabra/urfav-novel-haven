...
import {
  getRevenueSummary, getRevenueTimeseries, getTopNovels, getTopChapters,
  type RevenueBucket, type RevenueTimePoint, type TopNovelRow, type TopChapterRow,
...
export function AuthorRevenuePanel() {
  ...
  const topNovelsQ = useQuery({ queryKey: ["author-top-novels"], queryFn: () => getTopNovels(5, 90) });
  const topChaptersQ = useQuery({ queryKey: ["author-top-chapters"], queryFn: () => getTopChapters(5, 90) });
  ...
  return (
    <section className="mb-6">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        ...
        <TopChaptersCard rows={topChaptersQ.data ?? []} locale={locale} label={t("rev.topChapters")} />
      </div>
    </section>
  );
}
...
function StatCard({ icon: Icon, label, value, accent }: {
  ...
  return (
    <div className={`rounded-xl border p-3 ${accent ? "border-primary/40 bg-primary/5" : "border-border/40 bg-surface/40"}`}>
...
function RevenueBars({ data, locale }: { data: RevenueTimePoint[]; locale: string }) {
  ...
  if (data.length === 0) return <div className="py-8 text-center text-sm text-muted-foreground">—</div>;
  return (
    <div className="flex h-32 items-end gap-1">
      {data.map((d) => {
        ...
        const label = new Date(d.bucket_start).toLocaleDateString(locale, { month: "short", day: "numeric" });
        return (
          <div key={d.bucket_start} className="group relative flex flex-1 flex-col justify-end"
               title={`${label} · ${d.coins} (${d.tip_coins} tips, ${d.unlock_coins} unlocks)`}>
            <div className="flex flex-col overflow-hidden rounded-t" style={{ height: `${Math.max(2, h)}%` }}>
...
function TopNovelsCard({ rows, locale, label }: { rows: TopNovelRow[]; locale: string; label: string }) {
  return (
    <div className="rounded-2xl border border-border/40 bg-surface/40 p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-bold"><BookOpen className="h-4 w-4 text-primary" />{label}</div>
      {rows.length === 0 ? (
        ...
        <ul className="space-y-2">
          {rows.map((r) => (
            <li key={r.novel_id}>
              <Link to="/novels/$slug" params={{ slug: r.slug }} className="flex items-center gap-3 rounded-lg p-2 hover:bg-background/40">
                <img src={coverUrl(r.cover_url)} alt="" className="h-12 w-9 shrink-0 rounded object-cover" />
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    ...
                    {r.unlock_coins > 0 && <span className="ms-2">🔓 {r.unlock_coins.toLocaleString(locale)}</span>}
                  </div>
                </div>
                <div className="shrink-0 text-sm font-black text-primary tabular-nums">{r.coins.toLocaleString(locale)}</div>
              </Link>
            </li>
          ))}
        </ul>
...
function TopChaptersCard({ rows, locale, label }: { rows: TopChapterRow[]; locale: string; label: string }) {
  return (
    <div className="rounded-2xl border border-border/40 bg-surface/40 p-4">
      <div className="mb-3 flex items-center gap-2 text-sm font-bold"><FileText className="h-4 w-4 text-primary" />{label}</div>
      {rows.length === 0 ? (
        <div className="py-6 text-center text-sm text-muted-foreground">—</div>
      ) : (
        <ul className="space-y-2">
          {rows.map((r) => (
            <li key={r.chapter_id}>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-bold">{r.title || `#${r.chapter_number}`}</div>
