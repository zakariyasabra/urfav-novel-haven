...
export function ChapterEditor({ novelId, chapterId, onSaved }: { novelId: string; chapterId?: string; onSaved?: (id: string) => void }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      <h1 className="mb-6 text-xl font-black md:text-2xl">{editing ? t("chEd.edit") : t("chEd.new")}</h1>
      ...
      <div className="grid gap-4 rounded-2xl border border-border/40 bg-surface/40 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Button type="button" onClick={aiTranslate} disabled={translating || !editing} variant="outline" size="sm">
            ...
            {translating ? t("chEd.translating") : (tab === "en" ? t("chEd.t.ar2en") : t("chEd.t.en2ar"))}
          </Button>
        </div>

        <div className="grid grid-cols-[110px_minmax(0,1fr)] gap-3">
          <label><div className="mb-1 text-xs font-semibold">{t("chEd.number")}</div>
            <input type="number" min={1} value={num} onChange={(e) => setNum(parseInt(e.target.value) || 1)} className="input" /></label>
          <label><div className="mb-1 text-xs font-semibold">{tab === "ar" ? t("chEd.title.ar") : t("chEd.title.en")}</div>
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="input" dir={tab === "ar" ? "rtl" : "ltr"} /></label>
        </div>

        <label className="block"><div className="mb-1 text-xs font-semibold">{tab === "ar" ? t("chEd.content.ar") : t("chEd.content.en")}</div>
          <textarea value={content} onChange={(e) => setContent(e.target.value)} rows={18}
            dir={tab === "ar" ? "rtl" : "ltr"}
            className="input resize-y font-[Amiri,serif] text-base leading-loose" placeholder={tab === "ar" ? t("chEd.ph.ar") : t("chEd.ph.en")} /></label>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <label><div className="mb-1 text-xs font-semibold">{t("chEd.status")}</div>
          ...
          {status === "scheduled" && (
            <label><div className="mb-1 text-xs font-semibold">{t("chEd.scheduleAt")}</div>
              <input type="datetime-local" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} className="input" /></label>
          )}
        </div>

        <div className="flex flex-wrap gap-2">
          <Button onClick={() => save(false)} disabled={busy} variant="secondary">{t("chEd.save")}</Button>
          <Button onClick={() => save(true)} disabled={busy}>{t("chEd.savePublish")}</Button>
        </div>
      </div>
