...
import { fetchReviews, fetchRatingDistribution, fetchMyReview, upsertReview, toggleReviewLike } from "@/lib/social-api";
import { useAuth } from "@/hooks/use-auth";
import { timeAgoAr } from "@/lib/format";
import { Button } from "@/components/ui/button";

export function ReviewsSection({ novelId, ratingAvg, ratingCount }: { novelId: string; ratingAvg: number; ratingCount: number }) {
  const { user } = useAuth();
  const qc = useQueryClient();
  const reviewsQ = useQuery({ queryKey: ["reviews", novelId], queryFn: () => fetchReviews(novelId) });
  const distQ = useQuery({ queryKey: ["rating-dist", novelId], queryFn: () => fetchRatingDistribution(novelId) });
  const mineQ = useQuery({ queryKey: ["my-review", novelId, user?.id], queryFn: () => fetchMyReview(novelId), enabled: !!user });
    ...
    <section className="mt-10">
      ...
      <div className="mb-6 grid gap-4 rounded-xl border border-border/40 bg-surface/40 p-4 md:grid-cols-[220px_1fr]">
        <div className="grid place-items-center gap-1 border-b border-border/40 pb-4 md:border-b-0 md:border-e md:pb-0 md:pe-4">
          ...
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((n) => (
              <Star key={n} className={`h-4 w-4 ${n <= Math.round(ratingAvg) ? "fill-gold text-gold" : "text-muted-foreground"}`} />
            ))}
          </div>
      ...
      {user ? (
        <div className="mb-6 rounded-xl border border-border/40 bg-surface/40 p-4">
          <div className="mb-2 text-sm font-bold">{mineQ.data ? "تحديث مراجعتك" : "اكتب مراجعتك"}</div>
          <div className="mb-3 flex items-center gap-1">
            {[1, 2, 3, 4, 5].map((n) => (
              <button key={n} type="button" onClick={() => setScore(n)} aria-label={`تقييم ${n}`}
                className="p-0.5">
                <Star className={`h-6 w-6 transition ${n <= score ? "fill-gold text-gold" : "text-muted-foreground hover:text-gold"}`} />
              </button>
            ))}
          </div>
          ...
          <div className="mt-2 flex justify-end">
            <Button onClick={submit} disabled={busy || score < 1} size="sm"
        ...
        <div className="mb-6 rounded-lg border border-border/40 bg-surface/40 p-3 text-center text-sm text-muted-foreground">
          <Link to="/auth" className="font-bold text-primary">سجل دخول</Link> لكتابة مراجعة
        </div>
      )}

      {/* List */}
      <div className="space-y-3">
        {(reviewsQ.data ?? []).filter((r) => r.review_body || r.review_title).map((r) => (
          <article key={r.id} className="rounded-xl border border-border/40 bg-surface/50 p-4">
            <header className="mb-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="text-sm font-bold">{r.profile?.display_name ?? r.profile?.username ?? "مستخدم"}</div>
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <Star key={n} className={`h-3 w-3 ${n <= r.score ? "fill-gold text-gold" : "text-muted-foreground"}`} />
                    ))}
                    <span className="ms-1 text-[11px] text-muted-foreground">· {timeAgoAr(r.created_at)}</span>
                  </div>
                </div>
              </div>
              <button onClick={() => like(r.id)}
            ...
            {r.review_body && <p className="whitespace-pre-line text-sm text-foreground/85">{r.review_body}</p>}
          </article>
        ))}
        {reviewsQ.data && reviewsQ.data.filter((r) => r.review_body || r.review_title).length === 0 && (
          <div className="rounded-lg border border-border/40 bg-surface/40 p-6 text-center text-sm text-muted-foreground">لا توجد مراجعات بعد. كن أول من يكتب مراجعة.</div>
        )}
      </div>
    </section>
  );
