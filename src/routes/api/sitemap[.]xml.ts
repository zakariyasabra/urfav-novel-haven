...
const BASE = (
  ...
  "https://urfav-novel-haven.lovable.app"
).replace(/\/$/, "");
    ...
    handlers: {
      GET: async () => {
        ...
        const urls: string[] = [
          ...
          ...(novels ?? []).map(
            (n) =>
              `<url><loc>${BASE}/novels/${n.slug}</loc><lastmod>${n.updated_at}</lastmod></url>`,
          ),
          ...(
            ...
            .map(
              (c) =>
                `<url><loc>${BASE}/novels/${c.novel!.slug}/${c.chapter_number}</loc><lastmod>${c.updated_at}</lastmod></url>`,
            ),
        ];
