...
function InventoryPage() {
  ...
  const shown = items.filter(
    (i) => (filter === "all" || i.category === filter) && (!search || i.title_ar.includes(search)),
  );
        ...
        <section className="mb-6 rounded-2xl border border-primary/30 bg-primary/5 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {equip.map((e) => (
              <div
                key={e.slot}
                className="flex items-center gap-2 rounded-full border border-primary/40 bg-background/60 px-3 py-1 text-xs"
              >
                <span className="font-semibold">
                  {CATEGORY_LABELS_AR[e.slot as MarketCategory] ?? e.slot}:
                </span>
          ...
          {shown.map((it) => {
            ...
            return (
              <div key={it.id} className={`rounded-xl border ${r.ring} bg-card/70 p-3`}>
                <div
                  className={`mb-1 flex items-center justify-between text-[10px] font-bold uppercase ${r.text}`}
                >
                ...
                <div className="mb-2 flex h-16 items-center justify-center rounded-lg bg-background/60 text-2xl">
                  {it.image_url ? (
                    <img src={it.image_url} alt="" className="max-h-16" />
                  ) : (
                    (it.icon ?? "🎁")
                  )}
                </div>
                  ...
                  >
                    {equipped ? (
                      <span className="inline-flex items-center gap-1">
                        <Check className="h-3 w-3" /> مُجهّز
                      </span>
                    ) : (
                      "تجهيز"
                    )}
