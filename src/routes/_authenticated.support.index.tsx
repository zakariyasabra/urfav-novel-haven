...
function SupportIndex() {
  ...
  async function submit() {
    if (!subject.trim() || !body.trim()) { toast.error(t("support.required")); return; }
    setBusy(true);
    try {
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-6 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <div className="min-w-0">
          <h1 className="flex items-center gap-2 text-2xl font-black md:text-3xl"><LifeBuoy className="h-6 w-6 text-primary" />{t("support.title")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("support.subtitle")}</p>
        </div>
        <Button onClick={() => setOpen((v) => !v)}><Plus className="me-1 h-4 w-4" />{t("support.new")}</Button>
      </header>

      {open && (
        <div className="mb-6 rounded-2xl border border-border/50 bg-surface/40 p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-xs font-semibold">{t("support.category")}
              <select value={cat} onChange={(e) => setCat(e.target.value as TicketCategory)}
                className="mt-1 h-10 w-full rounded-md border border-input bg-background/60 px-3 text-sm">
                {CATEGORIES.map((c) => <option key={c} value={c}>{t(`support.cat.${c}`)}</option>)}
              </select>
            </label>
            <label className="text-xs font-semibold sm:col-span-2">{t("support.subject")}
              <input value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={200}
                className="mt-1 h-10 w-full rounded-md border border-input bg-background/60 px-3 text-sm" />
            </label>
            <label className="text-xs font-semibold sm:col-span-2">{t("support.description")}
          ...
          <div className="mt-3 flex justify-end gap-2">
            ...
            <Button onClick={submit} disabled={busy}>{busy ? t("common.saving") : t("support.submit")}</Button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        {(q.data ?? []).map((tk) => (
          <Link key={tk.id} to="/support/$id" params={{ id: tk.id }}
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
              ...
              <span className={`shrink-0 rounded-md px-2 py-1 text-[11px] font-semibold ${statusColor[tk.status] ?? "bg-muted"}`}>
