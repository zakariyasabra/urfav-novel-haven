...
function WalletPage() {
  ...
  async function redeemCoupon() {
    ...
    if (data.expires_at && new Date(data.expires_at) < new Date()) return toast.error(t("wallet.coupon.expired"));
    toast.success(t("wallet.coupon.valid", { pct: data.discount_percent, bonus: data.bonus_coins }));
    setCode("");
  }

  const sub = subQ.data as null | { status: string; expires_at: string | null; plan: { name_ar: string; name_en: string | null } | null };
  const planName = sub?.plan ? (lang === "en" ? (sub.plan.name_en || sub.plan.name_ar) : sub.plan.name_ar) : t("wallet.subUnknownPlan");
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-primary/40 bg-gradient-to-br from-primary/25 via-surface-elevated to-surface p-6 shadow-elevated">
        ...
        <div className="relative">
          <div className="mb-1 text-xs uppercase tracking-widest text-primary">{t("wallet.balance")}</div>
          <div className="flex flex-wrap items-baseline gap-2">
            <span className="break-all text-4xl font-black tabular-nums sm:text-5xl">{(walletQ.data?.coins ?? 0).toLocaleString(locale)}</span>
      ...
      {sub && (
        <div className="mt-4 rounded-2xl border border-border/40 bg-surface/40 p-4">
          <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground">{t("wallet.subscription")}</div>
          <div className="mt-1 flex flex-wrap items-center justify-between gap-2">
            <div className="font-bold">{planName}</div>
            <div className="text-xs text-muted-foreground">
              {sub.status} — {t("wallet.subExpiresAt", { d: sub.expires_at ? new Date(sub.expires_at).toLocaleDateString(locale) : "—" })}
            </div>
          </div>
        </div>
      )}
          ...
          {(packagesQ.data ?? []).map((p) => {
            ...
            return (
              <div key={p.id} className={`relative rounded-2xl border p-4 transition-all hover:-translate-y-0.5 hover:border-primary ${p.is_popular ? "border-primary bg-primary/[0.06]" : "border-border/40 bg-surface/40"}`}>
                {p.is_popular && <span className="absolute -top-2 end-3 rounded-full bg-primary px-2 py-0.5 text-[10px] font-bold text-primary-foreground">{t("wallet.mostPopular")}</span>}
                <div className="flex items-baseline gap-2">
                  <Coins className="h-5 w-5 text-primary" />
                  <span className="text-2xl font-black">{p.coins.toLocaleString(locale)}</span>
                  {p.bonus_coins > 0 && <span className="text-xs text-primary">+{p.bonus_coins} {t("wallet.gift")}</span>}
                </div>
                <div className="mt-1 text-sm font-bold">{primary}</div>
                {secondary && <div className="text-xs text-muted-foreground">≈ {secondary}</div>}
                <Button className="mt-3 w-full" size="sm" onClick={() => setBuying({ coins: totalCoins, usdCents: usd, egpCents: egp })}>{t("wallet.buyBtn")}</Button>
              </div>
            );
          })}
          {(packagesQ.data?.length ?? 0) === 0 && (
            <div className="col-span-full rounded-2xl border border-dashed border-border/50 p-8 text-center text-sm text-muted-foreground">{t("wallet.noPackages")}</div>
          )}
        </div>
      </section>

      <section className="mt-8 rounded-2xl border border-border/40 bg-surface/40 p-4">
        <h2 className="mb-3 flex items-center gap-2 text-lg font-black"><TicketPercent className="h-5 w-5 text-primary" />{t("wallet.coupon.title")}</h2>
        <div className="flex gap-2">
          <input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder={t("wallet.coupon.placeholder")}
          ...
          <Button size="sm" onClick={redeemCoupon}>{t("wallet.coupon.apply")}</Button>
        </div>
        <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
          <Users className="h-3.5 w-3.5" />{t("wallet.coupon.refer")}
        </div>
      </section>
      ...
      <section className="mt-8">
        ...
        {(txQ.data?.length ?? 0) === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/60 bg-surface/30 p-12 text-center text-sm text-muted-foreground">{t("wallet.txEmpty")}</div>
        ) : (
          <div className="divide-y divide-border/40 rounded-2xl border border-border/40 bg-surface/40">
            {(txQ.data ?? []).map((t2) => (
              <div key={t2.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 p-3">
                <div className={`grid h-9 w-9 place-items-center rounded-full ${t2.status === "completed" ? "bg-emerald-500/15 text-emerald-500" : "bg-amber-500/15 text-amber-500"}`}>
                  {t2.status === "completed" ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownLeft className="h-4 w-4" />}
                </div>
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold">{t2.provider ?? t("wallet.txPayment")}</div>
                  <div className="truncate text-xs text-muted-foreground">{timeAgo(t2.created_at)}</div>
                </div>
                <div className="text-sm font-black tabular-nums">{(Number(t2.amount_cents) / 100).toFixed(2)} {t2.currency}</div>
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mt-8">
        <h2 className="mb-3 flex items-center gap-2 text-xl font-black"><History className="h-5 w-5 text-primary" />{t("wallet.coinHistory")}</h2>
        {(coinHistoryQ.data?.length ?? 0) === 0 ? (
          <div className="rounded-2xl border border-dashed border-border/60 bg-surface/30 p-12 text-center text-sm text-muted-foreground">{t("wallet.coinHistoryEmpty")}</div>
        ) : (
          <div className="divide-y divide-border/40 rounded-2xl border border-border/40 bg-surface/40">
            {(coinHistoryQ.data ?? []).map((r) => (
              <div key={r.id} className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 p-3">
                <div className={`grid h-9 w-9 place-items-center rounded-full ${r.amount > 0 ? "bg-emerald-500/15 text-emerald-500" : "bg-red-500/15 text-red-500"}`}>
                  {r.amount > 0 ? <ArrowUpRight className="h-4 w-4" /> : <ArrowDownLeft className="h-4 w-4" />}
                </div>
                <div className="min-w-0">
                  <div className="truncate text-sm font-bold">{r.note ?? kindLabel(r.kind, t)}</div>
                  <div className="truncate text-xs text-muted-foreground">{timeAgo(r.created_at)}</div>
                </div>
                <div className={`text-sm font-black tabular-nums ${r.amount > 0 ? "text-emerald-500" : "text-red-500"}`}>{r.amount > 0 ? "+" : ""}{r.amount.toLocaleString(locale)}</div>
              </div>
            ))}
          </div>
        )}
