...
export const Route = createFileRoute("/_authenticated/author/novels/$id")({
  head: () => ({
    meta: [{ title: "Manage novel — FAVNOL" }, { name: "robots", content: "noindex" }],
  }),
  ...
  async function deleteChapter(chId: string) {
    if (
      !(await confirmDialog({
        ...
        body: t("authNv.ch.deleteConfirm"),
    ...
    <div className="mx-auto max-w-5xl px-4 py-6">
      <header className="mb-6 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-muted-foreground">
            {n.is_published ? (
              <span className="text-emerald-500">{t("authNv.published")}</span>
            ) : (
              <span className="text-amber-500">{t("authNv.draft")}</span>
            )}
          </div>
        </div>
        <Button
      ...
      <details className="mb-6 rounded-xl border border-border/40 bg-surface/40 p-4">
        ...
        >
          <label className="block">
            <div className="mb-1 text-sm font-semibold">{t("authNv.fTitle")}</div>
            <input name="title" defaultValue={n.title} className="input" required />
          </label>
          <label className="block">
            <div className="mb-1 text-sm font-semibold">{t("authNv.fAuthor")}</div>
            <input name="author" defaultValue={n.author} className="input" />
          </label>
          <label className="block">
            <div className="mb-1 text-sm font-semibold">{t("authNv.fDesc")}</div>
            <textarea
              name="description"
          ...
          <label className="block">
            <div className="mb-1 text-sm font-semibold">{t("authNv.fStatus")}</div>
      ...
      <div className="mb-4 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <h2 className="truncate text-lg font-black">
          {t("authNv.chaptersN", { n: chaptersQ.data?.length ?? 0 })}
        </h2>
      ...
      <div className="space-y-2">
        {(chaptersQ.data ?? []).map((c) => (
          <div
            key={c.id}
            className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-border/40 bg-surface/30 p-3"
          >
            <div className="min-w-0">
              <div className="truncate text-sm font-bold">
                {t("authNv.ch.label", { n: c.chapter_number, t: c.title })}
              </div>
              <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                {c.status === "published" && (
                  <span className="flex items-center gap-1 text-emerald-500">
                    <CheckCircle2 className="h-3 w-3" />
                    {t("authNv.ch.published")}
                  </span>
                )}
                {c.status === "draft" && (
            ...
            <div className="flex shrink-0 items-center gap-1">
              <Button asChild size="icon" variant="ghost" className="h-8 w-8">
                <Link to="/author/novels/$id/chapters/$chapterId" params={{ id, chapterId: c.id }}>
