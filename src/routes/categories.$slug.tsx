...
function GenrePage() {
  ...
  const novelsQ = useQuery({ queryKey: ["novels-by-genre", slug, lang], queryFn: () => fetchNovelsByGenre(slug) });
  const g = genresQ.data?.find((x) => x.slug === slug);
  const name = g ? (lang === "en" ? (g.name_en || g.name_ar) : g.name_ar) : slug;
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <h1 className="mb-6 text-3xl font-black md:text-4xl">{t("categories.title")}: <span className="text-gradient-primary">{name}</span></h1>
