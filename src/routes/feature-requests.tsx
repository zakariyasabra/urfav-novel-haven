...
import {
  fetchFeatureRequests, submitFeatureRequest, fetchMyVotes, toggleVote,
  type FRStatus,
...
const STATUSES: (FRStatus | "all")[] = ["all", "submitted", "planned", "accepted", "in_progress", "completed", "rejected"];
...
function FRPage() {
  ...
  const votesQ = useQuery({ queryKey: ["fr-votes", user?.id], queryFn: fetchMyVotes, enabled: !!user });
  ...
  async function vote(id: string) {
    if (!user) { toast.error(t("fr.needAuth")); return; }
  ...
  async function submit() {
    if (!title.trim() || !desc.trim()) { toast.error(t("fr.required")); return; }
    setBusy(true);
    try {
    ...
    <div className="mx-auto max-w-4xl px-4 py-8">
      <header className="mb-6 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <div className="min-w-0">
          <h1 className="flex items-center gap-2 text-2xl font-black md:text-3xl">
            <Lightbulb className="h-6 w-6 text-primary" />{t("fr.title")}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">{t("fr.subtitle")}</p>
        </div>
        {user && <Button onClick={() => setOpen((v) => !v)}><Plus className="me-1 h-4 w-4" />{t("fr.new")}</Button>}
      </header>

      {open && (
        <div className="mb-6 rounded-2xl border border-border/50 bg-surface/40 p-4">
          <label className="block text-xs font-semibold">{t("fr.reqTitle")}
            <input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={200}
              className="mt-1 h-10 w-full rounded-md border border-input bg-background/60 px-3 text-sm" />
          </label>
          <label className="mt-3 block text-xs font-semibold">{t("fr.description")}
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={5} maxLength={3000}
              className="mt-1 w-full rounded-md border border-input bg-background/60 p-3 text-sm" />
          </label>
          <div className="mt-3 flex justify-end gap-2">
            <Button variant="ghost" onClick={() => setOpen(false)}>{t("common.cancel")}</Button>
            <Button onClick={submit} disabled={busy}>{busy ? t("common.saving") : t("fr.submit")}</Button>
          </div>
        </div>
      )}
        ...
        {STATUSES.map((s) => (
          <button key={s} onClick={() => setFilter(s)}
            className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold ${
              filter === s ? "border-primary bg-primary/10 text-primary" : "border-border/60 text-muted-foreground"
            }`}>
      ...
      <div className="space-y-2">
        {(q.data ?? []).map((r) => {
          ...
          return (
            <div key={r.id} className="grid grid-cols-[64px_minmax(0,1fr)_auto] items-start gap-3 rounded-xl border border-border/40 bg-surface/40 p-4">
              <button onClick={() => vote(r.id)}
                className={`flex h-16 w-16 flex-col items-center justify-center rounded-lg border-2 transition-all ${
                  voted ? "border-primary bg-primary/15 text-primary" : "border-border/60 hover:border-primary/60"
                }`}>
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 line-clamp-3 whitespace-pre-wrap text-sm text-muted-foreground">{r.description}</p>
                {r.admin_note && (
                ...
                <div className="mt-2 text-[11px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString(locale)}</div>
              </div>
