...
export const Route = createFileRoute("/marketplace")({
  ...
  head: () => ({
    meta: [
      { title: "المتجر — FAVNOL" },
      { name: "description", content: "متجر العملات: إطارات، ثيمات، شارات، ألقاب، وصناديق غامضة." },
    ],
  }),
});
...
function MarketplacePage() {
  ...
  useEffect(() => {
    void mkListCategories().then(setCats);
  }, []);

  useEffect(() => {
    setLoading(true);
    if (active === "daily") {
      void mkDailyShop().then((d) => {
        setDaily(d);
        setLoading(false);
      });
    } else {
      void mkListItems(active).then((r) => {
        setItems(r);
        setLoading(false);
      });
    }
  ...
  const tabs = useMemo(
    () => [
      { code: "daily" as const, label_ar: "المتجر اليومي", icon: "⏳" },
      ...cats.map((c) => ({ code: c.code, label_ar: c.label_ar, icon: c.icon ?? "" })),
    ],
    [cats],
  );
      ...
      <header className="mb-6 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            اصرف عملاتك على إطارات، ثيمات، ألقاب وصناديق غامضة
          </p>
        </div>
    ...
    <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {items.map((it) => {
        ...
        return (
          <div
            key={it.id}
            className={`group relative flex flex-col rounded-2xl border ${r.ring} bg-card/70 p-4 transition hover:-translate-y-0.5 ${r.glow}`}
          >
            <div
              className={`mb-2 flex items-center justify-between text-[10px] font-bold uppercase ${r.text}`}
            >
              <span>{r.label_ar}</span>
              {it.vip_only && (
                <span className="rounded-full bg-amber-500/20 px-2 py-0.5 text-amber-300">VIP</span>
              )}
            </div>
            <div className="mb-3 flex h-24 items-center justify-center rounded-xl bg-background/60 text-4xl">
              {it.image_url ? (
                <img src={it.image_url} alt="" className="max-h-24" />
              ) : (
                (it.icon ?? "🎁")
              )}
            </div>
            ...
            {it.duration_days ? (
              <div className="mt-2 text-[10px] text-muted-foreground">
                مدة: {it.duration_days} يوم
              </div>
            ) : null}
            {it.stock != null ? (
              <div className="mt-1 text-[10px] text-muted-foreground">
                متبقي: {Math.max(0, it.stock - it.stock_sold)}
              </div>
            ) : null}
          </div>
        );
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {items.map((it) => {
          ...
          return (
            <div
              key={it.item_id}
              className={`relative flex flex-col rounded-2xl border ${r.ring} bg-card/70 p-4 ${r.glow}`}
            >
              {it.discount_percent > 0 && (
                <span className="absolute -top-2 start-3 rounded-full bg-red-500 px-2 py-0.5 text-[10px] font-bold text-white">
                  -{it.discount_percent}%
                </span>
              )}
              ...
              <div className="mt-auto flex items-center justify-between">
                <div className="flex items-center gap-1 text-sm font-black text-primary">
                  <Coins className="h-4 w-4" />
                  {it.discount_percent > 0 && (
                    <span className="me-1 text-xs text-muted-foreground line-through">
                      {it.price_coins}
                    </span>
                  )}
