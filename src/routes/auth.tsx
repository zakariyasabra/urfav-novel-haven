...
function AuthPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    toast.success(mode === "signin" ? "تم تسجيل الدخول" : "تم إنشاء الحساب — تحقق من بريدك لتأكيد الحساب");
    ...
    <div className="mx-auto max-w-md px-4 py-16">
      <div className="mb-6 text-center">
        ...
        <h1 className="text-3xl font-black">
          {mode === "signin" ? "مرحباً بعودتك" : mode === "signup" ? "أهلاً بك" : "استعادة كلمة المرور"}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {mode === "signin" ? "سجل دخولك للاستمتاع بمكتبتك" : mode === "signup" ? "أنشئ حساباً لحفظ المفضلات والتعليقات" : "أدخل بريدك لإرسال رابط الاستعادة"}
        </p>
      </div>
      ...
      {mode !== "forgot" && (
        <div className="mb-4">
          ...
          <div className="my-4 flex items-center gap-3 text-xs text-muted-foreground">
            <div className="h-px flex-1 bg-border/60" />أو<div className="h-px flex-1 bg-border/60" />
          </div>
        </div>
      )}
      ...
      <form onSubmit={submit} className="space-y-3 rounded-2xl border border-border/60 bg-surface/40 p-6">
        ...
        <Button type="submit" disabled={busy} className="h-11 w-full bg-gradient-to-r from-primary to-primary-glow text-primary-foreground">
          {busy ? "..." : mode === "signin" ? "تسجيل الدخول" : mode === "signup" ? "إنشاء حساب" : "إرسال الرابط"}
        </Button>
        {mode === "signin" && (
        ...
        <div className="text-center text-sm text-muted-foreground">
          {mode === "forgot" ? (
            <button type="button" onClick={() => setMode("signin")} className="font-bold text-primary hover:underline">العودة لتسجيل الدخول</button>
          ) : (
            <>
