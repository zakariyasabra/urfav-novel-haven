...
function AuthorsTab() {
  ...
  const filterLabel = (s: "pending" | "approved" | "rejected" | "") =>
    s === ""
      ? t("admin.authors.filter.all")
      : s === "pending"
        ? t("admin.authors.filter.pending")
        : s === "approved"
          ? t("admin.authors.filter.approved")
          ...
          <div key={a.id} className="rounded-xl border border-border/40 bg-surface/40 p-4">
            ...
            {a.admin_note && (
              <div className="mb-2 text-xs text-muted-foreground">
                {t("admin.authors.note", { note: a.admin_note })}
              </div>
            )}
            {a.status === "pending" && (
              <div className="flex gap-2">
                <Button size="sm" onClick={() => act(a.id, "approve")}>
                  {t("admin.authors.approve")}
                </Button>
                <Button size="sm" variant="destructive" onClick={() => act(a.id, "reject")}>
                  {t("admin.authors.reject")}
                </Button>
              </div>
            )}
          </div>
...
function NovelsTab() {
  ...
  const q = useQuery({
    ...
    queryFn: () => fetchNovels({ sort: "newest" }),
  });
  ...
  async function del(id: string) {
    if (
      !(await confirmDialog({
        ...
        body: t("admin.confirm.deleteNovel"),
    ...
    try {
      await translateFn({
        data: {
          entity_type: "novel",
          entity_id: id,
          fields: ["title", "description", "author_display", "original_title", "translator"],
          target_lang: "en",
        },
      });
    ...
    <div>
      ...
      <div className="space-y-3">
        {(q.data ?? []).map((n) => (
          <div
            key={n.id}
            className="grid grid-cols-[56px_minmax(0,1fr)_auto] items-center gap-3 rounded-xl border border-border/40 bg-surface/40 p-3 sm:grid-cols-[80px_minmax(0,1fr)_auto]"
          >
            <img
              src={coverUrl(n.cover_url)}
              alt=""
              className="h-20 w-14 rounded object-cover sm:w-20"
            />
            <div className="min-w-0">
              <div className="truncate font-bold">{n.title}</div>
              <div className="truncate text-xs text-muted-foreground">
                {n.author} · {statusLabel(n.status)} · {formatViews(n.views_count)}{" "}
                {t("admin.novels.viewsSuffix")}
              </div>
            </div>
            <div className="flex shrink-0 gap-1.5">
              <Button
                size="sm"
                variant="outline"
                onClick={() => aiTranslate(n.id)}
                disabled={translating === n.id}
                aria-label={t("admin.ai.translate") || "ترجمة AI"}
              >
              ...
              <Button
                ...
                aria-label={t("admin.action.edit") as string}
              >
              ...
              <Button
                ...
                variant="outline"
                onClick={() => del(n.id)}
                aria-label={t("admin.action.delete") as string}
              >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xl font-black">
            {novelId ? t("admin.novels.editTitleModal") : t("admin.novels.new")}
          </h3>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <Input
            label={t("admin.form.slug") as string}
            value={form.slug}
            onChange={(v) => setForm({ ...form, slug: v })}
          />
          <Input
            label={t("admin.form.title") as string}
            value={form.title}
            onChange={(v) => setForm({ ...form, title: v })}
          />
          <Input
            label={t("admin.form.author") as string}
            value={form.author}
            onChange={(v) => setForm({ ...form, author: v })}
          />
          <Input
            label={t("admin.form.translator") as string}
            value={form.translator}
            onChange={(v) => setForm({ ...form, translator: v })}
          />
          <Input
            label={t("admin.form.coverUrl") as string}
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full resize-none rounded-md border border-input bg-background/60 p-3 text-sm"
            />
          </div>
          <div className="md:col-span-2">
...
function ChaptersTab() {
  ...
  useEffect(() => {
    if (!novelId && novelsQ.data?.[0]) setNovelId(novelsQ.data[0].id);
  }, [novelsQ.data]);
  ...
  async function del(id: string) {
    if (
      !(await confirmDialog({
        title: t("admin.confirm.title"),
        body: t("admin.confirm.deleteChapter"),
        confirmLabel: t("admin.confirm.confirmLabel"),
        danger: true,
      }))
    )
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-2">
        <select
          ...
          className="h-10 min-w-0 max-w-full flex-1 rounded-md border border-input bg-background/60 px-3 text-sm sm:flex-none"
        >
          {(novelsQ.data ?? []).map((n) => (
        ...
        <Button
          onClick={() => setEditing("new")}
          disabled={!novelId}
          className="shrink-0 bg-gradient-to-r from-primary to-primary-glow text-primary-foreground"
        >
          <Plus className="me-1 h-4 w-4" />
          ...
          >
            <div className="min-w-0">
              <div className="truncate font-bold">
                {t("admin.chapters.rowTitle", { n: c.chapter_number, title: c.title })}
              </div>
              <div className="text-xs text-muted-foreground">
                {formatViews(c.views_count)} {t("admin.novels.viewsSuffix")}
              </div>
            </div>
            <div className="flex shrink-0 gap-1.5">
              <Button
...
function ChapterForm({
  ...
  return (
    ...
    >
      <div
        className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-2xl border border-border/60 bg-surface p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-xl font-black">
            {chapterId ? t("admin.chapters.editTitle") : t("admin.chapters.new")}
          </h3>
          <button onClick={onClose}>
            <X className="h-5 w-5" />
          </button>
        ...
        <div className="grid gap-3">
          <div className="grid gap-3 md:grid-cols-3">
            <Input
              label={t("admin.chapters.number") as string}
              value={String(form.chapter_number)}
              onChange={(v) => setForm({ ...form, chapter_number: Number(v) || 0 })}
            />
            <div className="md:col-span-2">
              <Input
                label={t("admin.form.title") as string}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold">
              {t("admin.chapters.content")}
            </label>
            <textarea
              value={form.content}
              onChange={(e) => setForm({ ...form, content: e.target.value })}
              rows={16}
              className="w-full resize-y rounded-md border border-input bg-background/60 p-3 font-serif text-sm leading-loose"
            />
          </div>
...
function CommentsTab() {
  ...
  async function del(id: string) {
    if (
      !(await confirmDialog({
        title: t("admin.confirm.title"),
        body: t("admin.confirm.deleteComment"),
        confirmLabel: t("admin.confirm.confirmLabel"),
        danger: true,
      }))
    )
