...
import { User as UserIcon, Shield, Sparkles, Languages, Moon, Sun, Monitor, RotateCcw } from "lucide-react";
...
function ProfilePage() {
  ...
  async function doReset() {
    const ok = await confirmDialog({ title: t("prefs.reset"), body: t("prefs.resetConfirm"), confirmLabel: t("common.confirm") });
    if (!ok) return;
    reset();
    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="rounded-2xl border border-border/60 bg-surface/40 p-6">
        <div className="mb-6 flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap gap-2">
              {isAdmin && <span className="inline-flex items-center gap-1 rounded-md bg-primary/20 px-2 py-0.5 text-xs font-bold text-primary"><Shield className="h-3 w-3" />{t("profile.admin")}</span>}
        ...
        <div className="space-y-3">
          ...
          <Button onClick={save} disabled={busy} className="bg-gradient-to-r from-primary to-primary-glow text-primary-foreground">{t("common.saveChanges")}</Button>
        </div>
      </div>

      {/* Appearance & Language */}
      <section className="mt-6 rounded-2xl border border-border/60 bg-surface/40 p-6">
        <h2 className="mb-4 flex items-center gap-2 text-xl font-black"><Languages className="h-5 w-5 text-primary" />{t("prefs.section")}</h2>

        <div className="space-y-5">
          <div>
            <label className="mb-2 block text-xs font-semibold text-muted-foreground">{t("prefs.language")}</label>
          ...
          <div>
            <label className="mb-2 block text-xs font-semibold text-muted-foreground">{t("prefs.theme")}</label>
            <div className="grid grid-cols-3 gap-2">
              <ThemeChoice value="dark" current={theme} onClick={() => setTheme("dark")} icon={<Moon className="h-4 w-4" />} label={t("prefs.theme.dark")} />
              <ThemeChoice value="light" current={theme} onClick={() => setTheme("light")} icon={<Sun className="h-4 w-4" />} label={t("prefs.theme.light")} />
              <ThemeChoice value="system" current={theme} onClick={() => setTheme("system")} icon={<Monitor className="h-4 w-4" />} label={t("prefs.theme.system")} />
            </div>
          </div>

          <div className="flex items-center justify-between gap-3 border-t border-border/40 pt-4">
            <p className="text-xs text-muted-foreground">{user ? t("prefs.syncedNote") : t("prefs.guestNote")}</p>
            <Button variant="outline" size="sm" onClick={doReset}>
              <RotateCcw className="me-1 h-4 w-4" />{t("prefs.reset")}
            </Button>
          </div>
        </div>
      </section>
...
function ThemeChoice({ value, current, onClick, icon, label }: { value: string; current: string; onClick: () => void; icon: React.ReactNode; label: string }) {
